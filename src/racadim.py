...
def check_valid_cond(x,y,visited_points,closest_threshold):
    for pos in visited_points:
        if math.sqrt((pos.x-x)**2 + (pos.y-y)**2) < closest_threshold: 
            return False
    return True 

def find_intersection(grid,beam_x,beam_y):
    
    if grid[beam_x,beam_y] == 80:
        return beam_x,beam_y
    if grid[beam_x-1,beam_y] == 80:
        return beam_x-1,beam_y
    if grid[beam_x+1,beam_y] == 80:
        return beam_x+1,beam_y
    if  grid[beam_x,beam_y+1] == 80:
        ...
        while True and not path_found:
            ...
            if not stop_beam and beam_x > 0 and beam_x < grid_size[0] and beam_y > 0 and beam_y < grid_size[1]:
                ...
                if(grid[beam_x,beam_y] == 100):
                    ...
                    if check_valid_cond(beam_x,beam_y,visited_robot,10):
                        valid_ray = True
                    ...
                    stop_beam = True
                    intersect_point_x,intersect_point_y = find_intersection(grid,beam_x,beam_y)
                    if check_valid_cond(intersect_point_x,intersect_point_y,visited_robot,10):
                        valid_ray = True
                        plt.scatter(intersect_point_x,intersect_point_y,s = 80, color = 'purple')
...
if __name__ == "__main__":
    ...
    while not path_found:
        if robot_pos.shape[0] >= 1:
            ...
            print("Can not perform more ray casting") 
