...
def simplify_path(waypoints):
    ...
    open_list = [(0, start)]
    came_from = {}
    g_score = {point: float('inf') for point in map(tuple, waypoints)}
    g_score[start] = 0

    while open_list:
        current_g, current_point = heapq.heappop(open_list)
        ...
        for next_point in map(tuple, waypoints):
            if next_point == current_point:
                continue

            if check_collision(list(current_point), list(next_point)):
                continue

            tentative_g = g_score[current_point] + euclidean_distance(list(current_point), list(next_point))

            if tentative_g < g_score[next_point]:
                came_from[next_point] = current_point
                g_score[next_point] = tentative_g
                f_score = tentative_g + euclidean_distance(list(next_point), list(end))
                heapq.heappush(open_list, (f_score, next_point))
...
def online_experiments_main():
    ...
    reduced_path = simplify_path(path)
